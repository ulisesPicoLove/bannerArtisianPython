...
class BannerGenerator:
    ...
    def generate_banners(self, directory):
        banner_sizes = [
            '120x600', '160x600', '200x200', '240x400', '250x250', '250x360', '280x120',
            '300x50', '300x250', '300x600', '300x1050', '320x50', '320x100', '320x480',
            '336x280', '468x60', '580x400', '600x740', '600x800', '728x90', '920x250',
            '930x180', '930x1080', '970x90', '970x250', '980x120'
        ]
    ...
    def process_files(self, source_dir, target_dir, size):
        image_files = glob.glob(os.path.join(source_dir, '*.{jpeg,jpg}'), recursive=True)
        total_size = 0
    ...
    def optimize_images(self, target_dir, image_files, size):
        ...
        for index, file_path in enumerate(image_files):
            optimized_file_path = os.path.join(target_dir, f"{size.upper()}_0{index + 1}.jpg")
